				...
				for ($i = 0; $i < $count2; $i++) {
					if (isset($_SESSION["player_tag"]) && isset($_SESSION["id"])) {
						...
						if ($_SESSION["player_tag"] == $tData[$i]["m.player_tag"]){
							echo ('<tr bgcolor="#0000FF"><td><a href="profile.php?user="' .$mData[$i]["m.member_id"]. '"">' .($tData[$i]["m.player_tag"]). '</a></td><td>' .($wData[$i]["mg.wins"]). '</td><td>' .($lData[$i]["mg.losses"]). '</td><td>' .($pData[$i]["p.name"]). '</td></tr>');
						}else{
