				...
				for ($i = 0; $i < $count2; $i++) {
					if (isset($_SESSION["player_tag"]) && isset($_SESSION["id"])) {
						...
						if ($_SESSION["player_tag"] == $tData[$i]["m.player_tag"]){
							echo ('<tr bgcolor="#FFCC99"><td><a href="profile.php?user=' .($mData[$i+1]["m.member_id"]). '">' .($tData[$i]["m.player_tag"]). '</a></td><td>' .($wData[$i]["mg.wins"]). '</td><td>' .($lData[$i]["mg.losses"]). '</td><td>' .($pData[$i]["p.name"]). '</td></tr>');
						}else{
